...
- **Cloudinary** (cloud `dvgk1ocvv`, upload preset `pje_treino`, conta no plano gratuito) é usado em `pagina4.html` e `detalhe.html` para upload de anexos (`/auto/upload`), retornando a URL que é armazenada no array `documentos` salvo no Firebase — mas **só para PDFs**. Vídeos (`.mp4/.avi/.mov/.mkv/.webm`) nunca são enviados de fato: `gravarDocumento()` detecta a extensão de vídeo e simula o upload (`arquivos.push({ nome, url: '' })`), sem chamar o Cloudinary. Isso é proposital — o plano gratuito do Cloudinary rejeita (HTTP 413) vídeos grandes mesmo dentro do limite de 300MB antigo do client, e como é uma ferramenta de treino, não há necessidade de guardar/baixar o vídeo de verdade depois. `downloadDoc()` já trata isso: para nomes de vídeo faz só `window.open(url, '_blank')`, então uma `url` vazia simplesmente abre uma aba em branco.
  - Limites de tamanho aplicados no client antes do upload: PDFs até 10MB (upload real), vídeos até 200MB (limite fictício, só para simular a validação — acima disso a página recusa antes mesmo de "simular").
...
- Em `pagina4.html`, os arrays `tipificacoes` (dispositivos penais/artigos de lei, usados no modal "Vincular dispositivo penal") e `assuntosList` (assuntos hierárquicos no formato `ÁREA|categoria|subcategoria|assunto|`, usados na aba "Assuntos") são listas estáticas de dados de treinamento, não exaustivas. Ao adicionar um novo tipo penal, manter as duas listas em sincronia — cada entrada em `tipificacoes` normalmente ganha uma entrada correspondente em `assuntosList` na mesma categoria/lei.
- A lista de prioridades do processo (`<select id="select-prioridade">`, aba "Características") é um conjunto fixo de `<option>`s em `pagina4.html`; não há array JS nem fonte externa para essas opções.
